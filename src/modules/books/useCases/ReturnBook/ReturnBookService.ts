...
class ReturnBookService {
  async execute({ returnId, userId }: TData): Promise<object> {
    ...
    const rentedBook = await rentRepository.verifyIfRentExists({
      returnId,
    });
    ...
    const rentUserLibraryBook = await rentRepository.verifyIfRentExists({
      returnId,
    });
    ...
    await rentRepository.delete({ returnId });
